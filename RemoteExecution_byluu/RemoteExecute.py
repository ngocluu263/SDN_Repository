...
ssh.connect(config.vmip,port=22, username=config.vmusername, password=config.vmpassword)
...
stdin2,stdout2,stderr2=ssh.exec_command('sudo cd /root/cci-recite/ && sudo git name-rev --name-only HEAD')
r=stdout2.readlines()
print r
